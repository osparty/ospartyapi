...
/**
 * Redis pub/sub implementation of {@link PartyV2Bus}, built on the same container-plus-channel shape as
...
public class RedisPartyV2Bus implements PartyV2Bus {
	...
	public RedisPartyV2Bus(StringRedisTemplate redis, RedisConnectionFactory connectionFactory,
	...
	private void publish(Signal signal) {
		try {
			...
			log.debug("Party V2 bus publish failed for {}: {}", signal.room(), e.toString());
		}
	}
	...
	private void onSignal(String payload) {
		try {
			...
			switch (signal.type()) {
				...
				default -> log.debug("Party V2 bus: unknown signal {}", signal.type());
			}
		}
		catch (Exception e) {
			log.debug("Party V2 bus signal handling failed: {}", e.toString());
		}
	}
