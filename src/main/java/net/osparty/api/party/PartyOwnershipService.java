...
/**
 * Which node owns a given live-party room (PARTY_V2_MIGRATION.md §5/§10). A room is owned by exactly one
 * node at a time; ownership is an atomic {@code SET NX} lock in the {@code pv2:} keyspace with a short TTL,
 * so a dead owner's lock expires and the room is free to be re-claimed.
 *
 * <p>The {@code pv2:} key prefix is a <em>data</em> contract, not a name — it deliberately outlived the
 * rename away from "V2". Two pods disagreeing about the prefix during a rolling deploy would each see the
 * other's rooms as unowned, and both would claim them: the exact split-brain this lock exists to prevent.
 * Changing it needs every pod stopped, not a rolling restart.
 *
