...
public class RedisPartyOwnershipService implements PartyOwnershipService {
	...
	public Claim claim(String room) {
		try {
			...
			log.warn("Party claim({}) failed, assuming local ownership: {}", room, e.toString());
			return Claim.CLAIMED;
		}
	}
	...
	public Optional<Owner> lookup(String room) {
		try {
			...
			log.debug("Party lookup({}) failed: {}", room, e.toString());
	...
	public boolean handoverPending(String room) {
		try {
			...
			log.debug("Party handoverPending({}) failed: {}", room, e.toString());
			return false;
		}
	}
		...
		try (Cursor<String> cursor = redis.scan(ScanOptions.scanOptions()
			...
			while (cursor.hasNext()) {
				...
				if (repaired != null && repaired == 1L) {
					log.warn("Party reclaim: {} had no expiry; applied one", room);
				}
				// Deliberately not claim(): that rewrites the party hash and pushes its TTL out. A reclaimed
				// room holds no PartyRoom, so nothing renews its lock — it expires, the next scan
				...
				if (claimed.size() >= MAX_RECLAIMS_PER_SCAN) {
					...
					log.info("Party reclaim: hit the {}-room cap, leaving the rest for the next scan",
			...
			log.debug("Party reclaim scan failed: {}", e.toString());
		}
		return claimed;
	}
	...
	public boolean ownedBySelf(String room) {
		try {
			...
			log.debug("Party ownedBySelf({}) failed, assuming still owner: {}", room, e.toString());
			return true;
		}
	}
	...
	private boolean exec(RedisScript<Long> script, String room, String... argv) {
		try {
			...
			log.debug("Party ownership script failed for {}: {}", room, e.toString());
			return true;
		}
	}
	...
	private void writePartyMeta(String room) {
		try {
			...
			log.debug("Party meta write failed for {}: {}", room, e.toString());
		}
	}
}
