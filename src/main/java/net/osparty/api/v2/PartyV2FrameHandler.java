...
public class PartyV2FrameHandler {
	...
	public PartyV2FrameHandler(PartyV2Manager manager, ObjectMapper mapper) {
	...
	public void onOpen(PartySession session) {
		...
		log.info("Party V2 WS connected: session={}", session.id());
	}
	...
	private void handleHost(Ctx ctx, Inbound in) {
		...
		log.info("Party V2 host: session={} room={} member={}", ctx.session.id(), in.room(), ctx.memberId);
	}
	...
	private void handleJoin(Ctx ctx, Inbound in) {
		...
		log.info("Party V2 join: session={} room={} member={}", ctx.session.id(), in.room(), ctx.memberId);
	}
	...
	private void sendUnowned(Ctx ctx, String room, boolean invited) {
		...
		log.info("Party V2 join refused: session={} room={} — no such room", ctx.session.id(), room);
