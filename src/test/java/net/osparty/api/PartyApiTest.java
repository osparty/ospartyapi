...
@TestPropertySource(properties = "app.rate-limit.interval-ms=0") // exercise the contract without throttling
class PartyApiTest
{
	@Autowired
	private MockMvc mvc;
	...
	void aHostCanOnlyHaveOneAd() throws Exception
	{
		...
		mvc.perform(get("/api/v1/parties"))
			.andExpect(status().isOk())
			// exactly one ad for this host, and it's the latest (tob)
