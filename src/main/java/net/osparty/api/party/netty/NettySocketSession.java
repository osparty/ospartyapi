...
/**
 * A {@link SocketSession} backed by a Netty channel, optionally tagged as one channel of a merged
 * connection.
 *
...
final class NettySocketSession implements SocketSession {
	/** No tag: this session owns its channel outright, as on the board's own endpoint. */
	...
	/**
	 * A genuine text frame, not bytes that happen to be UTF-8: the ad board's own endpoint answers clients
	 * that predate compression, and those read text. Sending binary to one of them would be silently ignored.
	 *
